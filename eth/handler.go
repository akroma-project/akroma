...
import (
	...
	"github.com/akroma-project/akroma/eth/fetcher"
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Akroma peer", "peer", id)
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping Akroma protocol")
	...
	log.Info("Akroma protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Akroma peer connected", "name", p.Name())
	...
	if err := p.Handshake(pm.networkID, td, hash, genesis.Hash()); err != nil {
		p.Log().Debug("Akroma handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Akroma peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Akroma message handling failed", "err", err)
			return err
		}
	}
}
...
type NodeInfo struct {
	Network    uint64              `json:"network"`    // Akroma network ID 200625
