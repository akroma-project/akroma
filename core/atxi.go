...
func WriteBlockAddTxIndexes(config *params.ChainConfig, indexDb ethdb.Database, block *types.Block) error {
	batch := indexDb.NewBatch()
	if _, err := putBlockAddrTxsToBatch(config, batch, block); err != nil {
		return err
	}
	return batch.Write()
}
...
func putBlockAddrTxsToBatch(config *params.ChainConfig, putBatch ethdb.Batch, block *types.Block) (txsCount int, err error) {
	for _, tx := range block.Transactions() {
		...
		log.Trace("Tx Tracer:", "to", to, "from", from, "block", block.Number(), "hash", tx.Hash)
...
func BuildAddrTxIndex(bc *BlockChain, chainDB, indexDB ethdb.Database, startIndex, stopIndex, step uint64) error {
	if bc.atxi == nil {
		log.Error("atxi not enabled for blockchain")
	...
	log.Info("Address/tx indexing (atxi)", "start:", startIndex, "stop:", stopIndex, "step:", step)
	...
	for i := startIndex; i < stopIndex; i = i + step {
		...
		txsCount, err := bc.WriteBlockAddrTxIndexesBatch(bc.Config(), indexDB, i, i+step, step)
		if err != nil {
			bc.atxi.Progress.LastError = err
			return err
		}
		...
		log.Info("Transacton Indexing Status", "block", i+step, "of", stopIndex, "tx", txsCount, "took", time.Since(stepStartTime).Round(time.Millisecond),
			"bps", float64(step)/time.Since(stepStartTime).Seconds(),
			"txps", float64(txsCount)/time.Since(stepStartTime).Seconds(),
			"mode", bc.GetAtxi().AutoMode)

		// Listen for interrupts, nonblocking
		select {
		case s := <-sigc:
			log.Warn("atxi build got interrupt: %s", s)
			return nil
		default:
		}
	...
	log.Info("Completed atxi-build", "duration", took.Round(time.Second), "blocks", stopIndex-startIndex, "blocks/sec",
		totalBlocksF/took.Seconds(), "tx", totalTxCount, "tx/sec", totalTxsF/took.Seconds())
...
func RmAddrTx(db ethdb.Database, tx *types.Transaction, config *params.ChainConfig, blockNumber *big.Int) error {
	if tx == nil {
		return nil
	}
	...
	txH := tx.Hash()
	msg, err := tx.AsMessage(types.MakeSigner(config, blockNumber))
	if err != nil {
		return err
	}
	from := msg.From()
